...
export default function DeliveryAgentTowerPage() {
  const { setScreen } = useScreen();
  const tower = useAgentTower(DELIVERY_AGENT_REGISTRY);
  ...
  useEffect(() => {
    setScreen({ module: "Delivery", page: "Agent Tower" });
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-foreground tracking-tight">Delivery · Agent Tower</h1>
              <p className="text-sm text-muted-foreground">
                15 named agents. Bounded scope. Deterministic where it matters — estimates, budgets, gates.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {filtered.map((a) => (
          <AgentCard
            ...
            detailHref={`/delivery/agents/${a.spec.id}`}
          ...
          {tower.decisions.slice(0, 12).map((d) => {
            ...
            return (
              <DecisionRow
                ...
                decision={d}
                spec={spec}
                traceHref={`/delivery/agents/${d.agentId}/decisions/${d.id}`}
