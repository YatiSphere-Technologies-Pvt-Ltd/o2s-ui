...
export default function PresalesAgentTowerPage() {
  ...
  useEffect(() => {
    setScreen({ module: "Pre-Sales", page: "Agent Tower" });
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-foreground tracking-tight">Pre-Sales · Agent Tower</h1>
              <p className="text-sm text-muted-foreground">
                18 named agents across the bid lifecycle — RFP intake to win/loss debrief.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {filtered.map((a) => (
          <AgentCard
            ...
            detailHref={`/presales/agents/${a.spec.id}`}
          ...
          {tower.decisions.slice(0, 12).map((d) => {
            ...
            return (
              <DecisionRow
                ...
                decision={d}
                spec={spec}
                traceHref={`/presales/agents/${d.agentId}/decisions/${d.id}`}
