...
export default function LegalAgentDetailPage({ params }: PageProps) {
  const { id } = use(params);
  ...
  useEffect(() => {
    if (!specName) return;
    setScreen({ module: "Legal", page: "Agent detail", recordId: id, recordLabel: specName });
  ...
  if (!entry) {
    return (
      <div className="max-w-3xl mx-auto py-16 text-center">
        ...
        <Link href="/legal/agents" className="text-brand underline mt-3 inline-block">
          Back to tower
        </Link>
      </div>
    );
          ...
          <ul className="bg-card border border-border rounded-xl overflow-hidden">
            {decisions.map((d) => (
              <DecisionRow
                ...
                decision={d}
                spec={entry.spec}
                traceHref={`/legal/agents/${id}/decisions/${d.id}`}
